...
async function fetchRoundWebContractData(
  ...
  const [profilesForRound, holes, creatorIdeas, coachingIdeas] = await Promise.all([
    ...
    rest<RoundHoleRow[]>(
      context,
      `holes?select=hole_number,par,score,fairway_result,gir_result,putts,chip_count,bunker_shot,sand_save,penalty_strokes,player_notes,voice_transcript&round_id=eq.${encodeURIComponent(roundId)}&order=hole_number.asc`,
    ),
  ...
  const scorecard: ScorecardEntry[] = holes
    ...
    .map((hole) => ({
      ...
      voice_transcript: permission.granted_at ? nullableString(hole.voice_transcript) : null,
    }));
